...
class PostgresSaver:
    ...
    def _save_genre(self, data: list) -> None:
        ...
        query = "INSERT INTO content.genre (id, name, description, created_at, updated_at) VALUES %s ON CONFLICT DO NOTHING"
    ...
    def _save_film_work(self, data: list) -> None:
        cur = self.connection.cursor()
        batch = [[row.id, row.title, row.description, row.creation_date, row.rating, row.type, row.file_path, row.created_at, row.updated_at] for row in data]
        query = "INSERT INTO content.film_work (id, title, description, creation_date, rating, type, file_path, created_at, updated_at) VALUES %s ON CONFLICT DO NOTHING"
        execute_values(cur, query, batch, template=None) 

    def _save_genre_film_work(self, data: list) -> None:
        cur = self.connection.cursor()
        batch = [[row.id, row.film_work_id, row.genre_id, row.created_at] for row in data]
        query = "INSERT INTO content.genre_film_work (id, film_work_id, genre_id, created_at) VALUES %s ON CONFLICT DO NOTHING"
        execute_values(cur, query, batch, template=None) 
    ...
    def _save_person_film_work(self, data: list) -> None:
        ...
        query = "INSERT INTO content.person_film_work (id, role, film_work_id, person_id, created_at) VALUES %s ON CONFLICT DO NOTHING"
        execute_values(cur, query, batch, template=None) 
