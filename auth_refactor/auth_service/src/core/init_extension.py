...
jwt_redis_blocklist = redis.Redis(host=settings.redis_host, port=settings.redis_port, db=0, decode_responses=True)

rate_limit_db = redis.Redis(host=settings.redis_host, port=settings.redis_port, db=1)
...
def init_trace(app: Flask):
    resource = Resource(attributes={"service.name": app.name})
    trace.set_tracer_provider(TracerProvider(resource=resource))
    trace.get_tracer_provider().add_span_processor(
        BatchSpanProcessor(
            JaegerExporter(agent_host_name=settings.jaeger_agent_host, agent_port=int(settings.jaeger_agent_port))
        )
    )
